...
use D4T\Admin\Demos\Http\Widgets\DailyDrawdownMetricsCard;
    ...
    public function index(Content $content)
    {
        return $content
            ...
            ->body(function (Row $row) {
                ...
                $metMaxDown = new DailyDrawdownMetricsCard("Max Drawdown","$40,00.00","58","%");
                $metMaxDown->badge(new Badge(StyleClassType::SUCCESS,"Passing"));
                $row->column(3, $metMaxDown);

                $metMaxMailyDown = new DailyDrawdownMetricsCard("Max Daily Drawdown","$40,00.00","58","%");
                $metMaxMailyDown->badge(new Badge(StyleClassType::SUCCESS,"Passing"));
                $row->column(3, $metMaxMailyDown);

                $metDaysTraded = new DailyDrawdownMetricsCard("Days Traded","0 Days","Days ago","0");
                $metDaysTraded->badge(new Badge(StyleClassType::SUCCESS,"Passing"));
                $row->column(3, $metDaysTraded);

                $metEndDate = new DailyDrawdownMetricsCard("End Date","10/10/2023","Days left","30");
//                $metEndDate->badge(new Badge(StyleClassType::SUCCESS,"Passing"));
                $row->column(3, $metEndDate);
            })
            ->body($this->newline())
            ->body(function (Row $row) {
